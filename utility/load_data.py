...
class Data_for_RecEraser_MF(object):
    def __init__(self, data_path, batch_size, part_type=1, part_num=15, part_T=1):
        self.batch_size = batch_size
        self.path = data_path
        self.train_normal = pd.read_csv(data_path + "/train_normal.csv", index_col=None)
        self.train_random = pd.read_csv(data_path + "/train_random.csv", index_col=None)
        ...
        if self.part_type != 0:
            try:
                ...
                if part_type == 2:
                    CC_, self.C_U, self.C_I = data_partition_2_withpath(self.path, train_all, part_num, part_T)
                    if isinstance(CC_, tuple):
                        self.C, self.C_itr = CC_[0], CC_[1]
                if part_type == 3:
                    ...
                    if isinstance(CC_, tuple):
                        self.C, self.C_itr = CC_[0], CC_[1]
                if part_type == 4:
                    CC_, self.C_U, self.C_I = data_partition_4_withpath(self.path, train_all, part_num, part_T)
                    if isinstance(CC_, tuple):
                        self.C, self.C_itr = CC_[0], CC_[1]
            ...
            try:
                ...
                if part_type == 1:
                    ...
                    if isinstance(CC_, tuple):
                        self.C, self.C_itr = CC_[0], CC_[1]
                if part_type == 2:
                    CC_, self.C_U, self.C_I = data_partition_2_withpath(self.path, train_all, part_num)
                    if isinstance(CC_, tuple):
                        self.C, self.C_itr = CC_[0], CC_[1]
                if part_type == 3:
                    CC_, self.C_U, self.C_I = data_partition_3_withpath(self.path, train_all, part_num, part_T)
...
class Data_for_BPR(object):
    ...
    def _build_pos_neg_pairs(self):

        train_all = pd.concat([self.train_normal, self.train_random], axis=0, ignore_index=True)
        user_groups = train_all.groupby('user')
        self.bpr_train_data = []

        for user, group in user_groups:
            pos_items = group[group['label'] == 1]['item'].values
            neg_items = group[group['label'] == 0]['item'].values

            if len(pos_items) > 0 and len(neg_items) > 0:

                neg_sampled = np.random.choice(neg_items, size=len(pos_items), replace=len(neg_items) < len(pos_items))
                for pos_item, neg_item in zip(pos_items, neg_sampled):
                    self.bpr_train_data.append([user, pos_item, neg_item])

        self.bpr_train_data = np.array(self.bpr_train_data)

    def set_train_mode(self, mode='full'):
        self.train_mode = mode
        if mode == 'full':
            self.train = self.bpr_train_data.copy()
        else:

            train_normal_pos_neg = []
            user_groups = self.train_normal.groupby('user')
            for user, group in user_groups:
                pos_items = group[group['label'] == 1]['item'].values
                neg_items = group[group['label'] == 0]['item'].values
                if len(pos_items) > 0 and len(neg_items) > 0:
                    neg_sampled = np.random.choice(neg_items, size=len(pos_items),
                                                   replace=len(neg_items) < len(pos_items))
                    for pos_item, neg_item in zip(pos_items, neg_sampled):
                        train_normal_pos_neg.append([user, pos_item, neg_item])
            self.train = np.array(train_normal_pos_neg)
    ...
    def batch_generator(self):
        ...
        if self.train_loader is None:
            ...
            data = torch.from_numpy(self.train[idx])
            self.train_loader = torch.utils.data.DataLoader(
                data, batch_size=self.batch_size, shuffle=True, drop_last=False
            )
        return self.train_loader
